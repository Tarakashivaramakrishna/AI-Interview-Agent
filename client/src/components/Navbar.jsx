...
const Navbar = () => {
  ...
  return (
    <div className="bg-[#f3f3f3] flex justify-center px-4 pt-6">
      <motion.div initial={{opacity:0,y:-40}} animate={{opacity:1,y:0}} transition={{duration:0.3}} className="w-full max-w-6xl bg-white rounded-[24px] shadow-sm border border-gray-200 px-8 py-4 
            ...
            <div className="relative">
               <button onClick={()=>{
                ...
                setShowUserPopUp(!showUserPopUp);setShowCreditPopUp(false);}} className="w-9 h-9 bg-black text-white rounded-full flex items-center justify-center font-semibold">
                    {userData?.user?.name ? userData.user.name.slice(0,1).toUpperCase():<FaUserAstronaut size={16}/>}
               </button>
               {showUserPopUp && (
                    <div className="absolute  right-0 mt-3 w-48 bg-white shadow-xl border border-gray-200 rounded-xl p-4 z-50">
                        <p className="text-md etxt-blue-500 font-medium mb-1">{userData?.user?.name}</p>
                        ...
                        <button onClick={handleLogout} className="w-full text-left text-sm py-2 flex-items-center gap-2 text-red-500"><HiOutlineLogout size={20}/>Logout</button>
                    </div>
                    )}
            </div>
          </div>
